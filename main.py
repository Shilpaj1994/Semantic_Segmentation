...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer4a_in2 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1,
                                   ...
                                   kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                   kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    # skip connection (element-wise addition of upsampled 7th layer output and 4th layer of vgg)
    layer4a_out = tf.add(layer4a_in1, layer4a_in2)



    # upsample previous layer output
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # sess1 = saver.restore(save_path)
        # helper.video_test(sess=sess, logits=logits, keep_prob=keep_prob, image_pl=input_image)


if __name__ == '__main__':
    logits, input_image = run()

    pickle_out = open("lgt.pickle", "wb")
    pickle_out1 = open("img.pickle", "wb")

    pickle.dump(logits, pickle_out)
    pickle.dump(input_image, pickle_out1)
